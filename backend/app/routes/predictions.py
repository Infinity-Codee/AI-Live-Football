...
async def get_ai_analysis(match_id: int, db: AsyncSession = Depends(get_db)):
    """
    AI textual match analysis (Arabic + Turkish) generated by Google Gemini from
    the real live match data. Cached per fixture to save tokens. Returns empty
    strings (available=false) when Gemini is not configured or unavailable.
    """
    result = await db.execute(select(Match).where(Match.id == match_id))
    match = result.scalars().first()
    if not match:
    ...
    response = {
        ...
        "en": (analysis or {}).get("en", ""),
        "tr": (analysis or {}).get("tr", ""),
    }
    if analysis is not None:
        await cache.set(cache_key, response, settings.analysis_cache_ttl)
    return response
